...
import $eleConfig from '@client/pages/editor/ele-config'
/**
 * 编辑器数据状态存储
 */
const state = {
...
const actions = {
  ...
  copyPage({ commit }, uuid) {
    ...
    console.log("复制页面数据", data)
    commit('insertPage', data)
    commit('addHistoryCache', "复制页面")
  },
  ...
  deleteElement({ state, commit }, uuid) {
    ...
    commit('addHistoryCache', '删除组件')
  },
  // 重置样式
  resetElementCommonStyle({ commit }, style) {
    commit('resetElementCommonStyle', style)
    commit('addHistoryCache', '重置样式')
  },
  //重置画布
  resetPageData({ commit }) {
    //清空画布内容
    let activePage = getters.activePage(state)
    activePage.elements.splice(0, activePage.elements.length)
    commit('setActiveElementUUID', '')
    commit('addHistoryCache', '清空画布')
  },
  /**
   * 添加动画到元素上
   * @param commit
  ...
  addElementAnimate({ commit }, animationName) {
    ...
    commit('addHistoryCache', `添加${animationName}动画`)
  },
  /**
   * 删除动画
   * @param commit
...
const mutations = {
   ...
   * @param state
   */
  addHistoryCache(state, action) {
    console.log("addHistoryCache -> action", action)
    if (!action) action = '移动'
    if (action && action.includes('Ds-')) {
      // 如果是组件记录，则查找组件对应名称
      $eleConfig.forEach((v, i) => {
        $eleConfig[i].components.filter(vv => {
          if (vv.elName == action) {
            action = `添加${vv.title}组件`
            return vv.title
          }
        })
      })
