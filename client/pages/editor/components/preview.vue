    ...
    <div class="preview-info-wrapper">
      ...
      <div class="info-form-wrapper">
        <div class="info-form-l">
            <el-checkbox @change="changeStatus">设置微信分享样式</el-checkbox>
        </div>
      </div>
      <div class="share-wx-config-wrapper marginB30" v-show="wxStatus">
        <div class="info-form-r page-info">
          <div class="page-cover">
            <div v-if="pageData.shareConfig.coverImage">
...
<script>
    ...
    created(){
        if(this.pageData.shareConfig){
          this.wxStatus = this.pageData.shareConfig.shareWx
        }else{
          this.$message.error("预览出错了，请刷新页面重试")
        }
    },
    watch: {
        wxStatus(newValue){
            this.pageData.shareConfig.shareWx = newValue
        }
    },
		methods: {
			/**
