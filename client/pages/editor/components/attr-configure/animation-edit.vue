          ...
          <el-scrollbar class="animate-choose-item">
            <div
              ...
              <span class="animate-choose-image"
                    :style="{backgroundPosition: `${animate.show_X}px ${animate.show_Y }px`}"
...
// import LoginVue from '../../../Login.vue'
	...
	export default {
      ...
      hoverPreviewAnimate(animation){
          Bus.$emit('RUN_ANIMATION_PREVIEW', this.activeElement.uuid, [{
            delay: 0,
            duration: 1,
            infinite: "",
            interationCount: 1,
		...
		methods: {
			...
			handleChooseAnimate(item) {
				...
				if (this.reSelectAnimateIndex === undefined) {
					...
					this.$store.dispatch('addHistoryCache','添加动画')
				}
			},
			/**
			 * 删除动画
       ...
       */
      // getAnimate(type){
      //   animateCssData.forEach(element => {
