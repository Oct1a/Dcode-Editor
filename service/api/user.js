...
module.exports = app => ({
  ...
  async updateUserStatus(id, state) {
    const { $model } = app;
    return await $model.user.findByIdAndUpdate(id, { $set: { status: state } });
  },
  /**
   * 更改用户权限
   */
  // api/user/updatePower
  async updateUserPower(id) {
    const { $model } = app;
    let user = await $model.user.findOne({ _id: id }).select('permissions')
    return await $model.user.findByIdAndUpdate(id, { $set: { permissions: user.permissions == 0 ? 1 : 0 } });
  },
  /**
   * 更新个人密码
   * @param {*} newPass
